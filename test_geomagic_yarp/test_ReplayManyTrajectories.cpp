 ...
 * Here, the simulated robot follows trajectories given by the partner on the geomagic touch arm robot.
 * During all the movement the partner has to maintain the black button, to allow the robot to learn it. It can learn many trajectory without limitation. After that, if we press the white button, we have to write the number of the trajectory we want to replay. Then, the robot will do the movement and geomagic touch arm robot will try to follow this movement.
...
public:
    ...
    { 
		...
		{
			cout << "end of the learned trajectory" << endl;
			fx.push_back(ftx);
			fy.push_back(fty);
			fz.push_back(ftz);
		...
		if((buttons[1]!=0) && (flagReplay==0))
		{
			if (fx.size() > 0)
			{
				cout << "give the number of replay between 0 and " << fx.size() -1 << endl;
				cin >> nbReplay;
				if( (nbReplay < 0) || nbReplay > fx.size() - 1)
				{
					cout << "error of value, we replay the first one" << endl;
					nbReplay = 0;
				}
			 ...
			 else cout << "We cannot replay since we don't learn any movements." << endl;
